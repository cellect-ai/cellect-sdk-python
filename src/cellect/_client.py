...
from ._exceptions import CellectError, APIStatusError
...
class Cellect(SyncAPIClient):
    ...
    def __init__(
        ...
        *,
        ...
        """Construct a new synchronous Cellect client instance.

        This automatically infers the `api_key` argument from the `CELLECT_API_KEY` environment variable if it is not provided.
        """
        if api_key is None:
            api_key = os.environ.get("CELLECT_API_KEY")
        if api_key is None:
            raise CellectError(
                "The api_key client option must be set either by passing api_key to the client or by setting the CELLECT_API_KEY environment variable"
            )
        self.api_key = api_key

        if base_url is None:
            base_url = os.environ.get("CELLECT_BASE_URL")
        if base_url is None:
...
class AsyncCellect(AsyncAPIClient):
    ...
    def __init__(
        ...
        *,
        ...
        """Construct a new async AsyncCellect client instance.
